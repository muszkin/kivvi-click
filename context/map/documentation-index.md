...
| `AGENTS.md` | repo | Same for Codex, incl. "Current Shape" | canonical | rewritten `fe9c3fe` (2026-09-09, CON-1) | read-full | AGENTS.md itself flags R22 (stale format hook) | overview, risks |
...
| `.claude/settings.json` | tooling | PostToolUse formatter hook | supporting | unchanged since old stack | read-full | matches only `*.php`/`*.ts` — stale for Java/Vue (R22, also flagged in-file by `AGENTS.md`) | delivery, risks |
...
| `compose.yaml`, `compose.prod.yaml`, `mercure/Caddyfile` (inline comments) | ops | Operational rationale (Mercure internal-URL trick, volume isolation, forwarded headers) | canonical | 2026-09-09 | read-full | comments describe a still-coexisting old stack / "both stacks" / mirror a now-deleted `frankenphp/Caddyfile` — stale prose, not stale behaviour (R23) | architecture, delivery, risks |
| `.env`, `.env.example`, `.env.prod.docker.example` (key names only, values not read) | ops | Compose substitution defaults | canonical | `.env.prod.docker.example` still carries old-stack-only keys (`APP_SECRET`, `DEFAULT_URI`, `CADDY_MERCURE_*`, `POSTGRES_VERSION`) and "both stacks" framing | read-full (key names) | R23 | technology, risks |
