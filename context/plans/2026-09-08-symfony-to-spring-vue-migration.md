...
| e2e | required | `cd tests/e2e && E2E_BASE_URL=https://localhost:<port> npx playwright test <journey specs>` (confirmed command, new base URL) + `… tools/migration-verify/compare.mjs --dimension performance` (DEV-8 budgets; `/collect` p95 over 50 requests) | all tests green; budgets met | `<run>/waves/<wave>/e2e.md` + Playwright report |
