...
**Status:** approved (owner, 2026-09-08; coexistence strategy parallel rewrite confirmed). Revision 2026-09-08 by the orchestrator before wave-0: DEV-12 (404 error-page bodies) added and the compare tool moved to `tools/migration-verify/` so the oracle directory stays immutable; no scope change.
**Date:** 2026-09-08
...
- **Commands (introduced in wave-0 unless marked confirmed):** backend `cd backend && ./mvnw -q verify` (unit+integration+ArchUnit); frontend `cd frontend && npm run test` (Vitest), `npm run typecheck` (`vue-tsc --noEmit`), `npm run lint`, `npm run build`; stack `docker compose -p <lease> -f compose.next.yaml up -d --build --wait`; e2e `cd tests/e2e && E2E_BASE_URL=https://localhost:<port> npx playwright test <spec>` (confirmed command, new base URL); compare `node tools/migration-verify/compare.mjs --journey <id> --base https://localhost:<port>` (introduced in wave-0, kept outside the immutable oracle directory: replays the oracle `capture/scenarios.json`, writes wave evidence, applies masks and `normalize.json`, exits non-zero on regression).
...
- **Likely change surface:** `backend/**`, `frontend/**`, `compose.next*.yaml`, `tools/migration-verify/compare.mjs`, `.github/workflows/next-build.yml` (introduced CI job: `mvnw verify` + `npm run build`).
...
| DEV-13 | event-stream (POST /collect `occurred_at`) | contract | the Java parser accepts ISO-8601 instants/offset date-times/local date-times only, narrower than PHP's `new DateTimeImmutable($s)` grammar (space-separated timestamps, relative strings, `@unix`) | strict parsing is safer; no client and no oracle step sends `occurred_at` today | contract verifier ignores `occurred_at` grammar differences beyond the oracle's own cases | Piotr Mucha | contract stage (revisit when k.js is built, PIO-80) | **RETIRED 2026-09-09 (CON-1): unused — no oracle step or spec ever sends `occurred_at`; verified byte-identical without a mask (final contract cohort).**
...
| contract | required (leading) | `node tools/migration-verify/compare.mjs --journey <id> --base https://localhost:<port> --dimension contract` (introduced wave-0) | 0 unmasked differences after `normalize.json` + DEV masks; `db.json` deltas equal after DEV-5 mapping | `<run>/waves/<wave>/contract.md` + `contract/<journey>/step-<n>.diff.json` |
| visual | required on every SPA step (no server-rendered pages remain) | `… tools/migration-verify/compare.mjs --dimension visual` (screenshots via `pixelmatch`-equivalent from Playwright `toHaveScreenshot` engine; a11y via `ariaSnapshot`; texts) | ≤ 0.5 % pixels after antialiasing tolerance and masks; a11y tree and texts exact | `<run>/waves/<wave>/visual.md` + `visual/<journey>/step-<n>/{diff.png,a11y.diff,texts.diff}` |
| e2e | required | `cd tests/e2e && E2E_BASE_URL=https://localhost:<port> npx playwright test <journey specs>` (confirmed command, new base URL; `events.spec.ts` runs with `--workers=1` — its exact-count assertions race with its own `/collect` tests under `fullyParallel`, a harness property identical on the old stack; orchestrator ruling 2026-09-08) + `… tools/migration-verify/compare.mjs --dimension performance` (DEV-8 budgets; `/collect` p95 over 50 requests) | all tests green; budgets met | `<run>/waves/<wave>/e2e.md` + Playwright report |
...
## Errata (recorded at CON-1, 2026-09-09)

- CUT-1 row: the command must combine both files — `docker compose -p kivvi-click --env-file .env.prod.docker -f compose.yaml -f compose.prod.yaml up -d --wait` (files renamed from `compose.next*` at CON-1); run old-stack compose commands only from the repository directory (project name = directory name).
- Ports: production publishes 23456 only; 23457 (HTTPS/HTTP3) was old-stack-only.
- Fixtures: implemented as Java classes under `click.kivvi.fixtures` (the decision ledger said JSON resources) — accepted.
- DEV-13 retired (see the deviation table); DEV-11 closed for login in wave-4, kept for landing step 5; DEV-12 covers customers 6, settings 10, import-wizard 10, shell-navigation 21.
- Actual wave outcomes: wave-3 needed three repair rounds (full K1 budget); wave-4 two; the wave-5 cohort was the final all-journey cohort. Cutover 2026-09-09 14:18:42Z after two rolled-back attempts (see the run ledger `cutover/`).
