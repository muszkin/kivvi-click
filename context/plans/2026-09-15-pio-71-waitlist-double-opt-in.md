...
| K18 | (brak) | `main` dołożyło własny blok pocztowy do `compose.portainer.yaml` (`a0c80a2`) — te same klucze Springa, ale pod **innymi** nazwami zmiennych stacka (`SMTP_HOST`, `SMTP_USERNAME`, `SMTP_PASSWORD`, `MAIL_FROM`), plus `KIVVI_BREVO_*` pod przyszłą synchronizację listy. | Przyjmujemy nazewnictwo z `main` we wszystkich trzech plikach (`compose.portainer.yaml`, `compose.prod.yaml`, `.env.prod.docker.example`). Ten ticket dokłada tylko `KIVVI_BASE_URL` i `KIVVI_UNSUBSCRIBE_SECRET`. Dwa równoległe zestawy nazw oznaczałyby, że zmienne ustawione w Portainerze nie są przez nic czytane — czyli dokładnie awaria z K14, tylnymi drzwiami. |
| K19 | (brak) | Nadawcą miało być `no-reply@kivvi.click`, ale `main` notuje, że zweryfikowanym nadawcą na uwierzytelnionej domenie jest `piotr@kivvi.click`, a wysyłka z czegokolwiek innego łamie wyrównanie DKIM. | Właściwość to `kivvi.mail.from` (żeby wiązała się z `KIVVI_MAIL_FROM` ze stacka), domyślnie `piotr@kivvi.click`. R1 planu mówi wprost, czym grozi pierwsza wysyłka z zimnej domeny bez poprawnego DKIM. |
| K20 | „Brak `spring.mail.host` przerywa start” (R5) — i na tym strażnik się kończył. | Po przyjęciu nazewnictwa z `main` host ma **wartość domyślną** (`smtp-relay.brevo.com`), więc nigdy nie jest pusty, a strażnik przestał mieć jak zadziałać. Poświadczenia defaultu nie mają, a `mail.smtp.auth` jest `true`: zapomniane `SMTP_PASSWORD` daje kontener zdrowy, stronę serwującą, zielony check po deployu i każdą wiadomość odbijającą się na AUTH przez 13 godzin — dokładnie ta cicha awaria, przed którą strażnik miał chronić. | Strażnik sprawdza też `spring.mail.username` i `spring.mail.password`, gdy uwierzytelnianie jest włączone. Host zachowuje użyteczny default; brakujący warunek dokładamy, zamiast usuwać default. |
