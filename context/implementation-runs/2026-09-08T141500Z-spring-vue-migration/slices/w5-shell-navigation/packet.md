...
- Backend: `ShellPagesIT` — `@ParameterizedTest` over the 25 old-stack URLs from `tests/Controller/PanelPagesTest.php` asserting the SPA document is 200 and the `GET /api/v1/{locale}/shell?route=<name>` payload carries the resolved `currentSection`/crumb marker; 404 for unknown locale/ids exactly as the old `PanelPagesTest` expects.
