...
- compare.mjs/capture.mjs freeze the page clock (`page.clock.setFixedTime`): under the verifier `performance.getEntriesByType("navigation")` stays EMPTY, `Date.now()` is constant and timers behave differently. Never base product behaviour on Navigation Timing or wall-clock deltas; prove behaviour under compare.mjs, not only in a plain browser (wave-3 campaigns repair-3).
...
- After the Docker data-root moved to the NAS (2026-09-09), Testcontainers Postgres startups time out when more than one Maven `verify` runs at once or host load is high: run exactly ONE `./mvnw -q verify` at a time, foreground, and never alongside other JVM-heavy jobs.
