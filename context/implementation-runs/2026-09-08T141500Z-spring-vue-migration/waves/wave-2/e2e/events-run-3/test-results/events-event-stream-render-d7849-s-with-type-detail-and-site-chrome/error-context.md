...
- Name: events.spec.ts >> event stream >> renders thirty rows with type, detail and site
- Location: specs/events.spec.ts:8:9
...
Locator:  locator('#event-stream .event-row')
Expected: 30
Received: 31
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('#event-stream .event-row')
