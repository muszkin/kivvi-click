# Verifier packet — wave <wave-id>, dimension <dimension>
...
- Wave SHA: <sha> · Journeys: <list> · Checkout: <path> (detached at the wave SHA; do not edit).
...
- Running stack for this SHA (if your dimension needs one): <url>, compose project <lease>, started by the orchestrator; do not start or stop stacks yourself.
- Evidence output dir: <run-dir>/waves/<wave-id>/<dimension>/ (write only here) and the summary file <run-dir>/waves/<wave-id>/<dimension>.md.
