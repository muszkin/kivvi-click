...
| Intake | A ticket or task brief is filed in Linear (issues, team `PIO`) with GitHub for pull requests, reviews and CI and meets the Definition of Ready below. `om-prepare-issue` files it with SDLC labels and the ready sections; `om-auto-manage-issues` reports what an existing ticket still lacks. | Anyone, `om-prepare-issue`, `om-auto-manage-issues` | Ticket exists and is ready, or its gaps are named on the ticket |
...
When `om-discover` has written `context/plans/product-brief.md`, its **Non-goals**, **Business rules**, and **Decisions** tables are protected the way `BACKWARD_COMPATIBILITY.md` protects contract surfaces. Each entry carries a stable id (`N01`, `R03`, `D07`), an owner, a status (`active` or `superseded`), a review-by date, and a required path for changing it. The rules:
...
4. `cd frontend && npm run format:check`
5. `cd frontend && npm run test -- --run`
6. `cd frontend && npm run test:integration -- --run`
7. `cd frontend && npm run build && rm -rf ../backend/src/main/resources/static && mkdir -p ../backend/src/main/resources/static && cp -r dist/. ../backend/src/main/resources/static/`
8. `cd backend && JAVA_HOME=$HOME/.cache/kivvi-toolchains/jdk-25 ./mvnw -B -q verify`
